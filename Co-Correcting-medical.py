...
class CoCorrecting(BasicTrainer, Loss): # 定义CoCorrecting类，继承自BasicTrainer和Loss
    ...
    def _compute_loss(self, outputA, outputB, target, target_var, index, epoch, i, parallel=False): # 计算损失的函数
        if epoch < self.args.stage1:
            ...
            onehot = onehot.numpy()
            self.new_y[index, :] = onehot  # 使用onehot作为新的y，硬标签
            # training as normal co-teaching
            forget_rate = self._rate_schedule(epoch)
            if self.args.loss_type == 'coteaching': # 如果损失类型是coteaching
                # 这里的lossA和lossB是两个模型的损失，ind_A_update和ind_B_update是两个模型更新的索引，ind_A_discard和ind_B_discard是两个模型丢弃的索引
                # pure_ratio_1和pure_ratio_2是两个模型选择的纯样本比例，pure_ratio_discard_1和pure_ratio_discard_2是两个模型丢弃的纯样本比例
                # outputA和outputB是两个模型的输出，target_var是目标变量，noise_or_not是噪声或非噪声，softmax是是否使用softmax
            ...
            elif self.args.loss_type == 'coteaching_plus': # 如果损失类型是coteaching_plus
                lossA, lossB, ind_A_update, ind_B_update, ind_A_discard, ind_B_discard, pure_ratio_1, pure_ratio_2, \
    ...
    def _cluster_data_into_subsets(self): # 将数据聚类到子集中
        ...
        # n_subsets是子集数，verbose是是否显示详细信息，random_state是随机种子，dim_reduce是降维
    ...
    def training(self): # 训练
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            # load y_tilde 加载y_tilde,即更新标签分布后的标签
            ...
            # top1的准确率,top1是指预测正确的样本数占总样本数的比例
    ...
    def train(self, epoch=0): # 训练，epoch默认为0，即从第0个epoch开始
        ...
        for i, (input, target, index) in enumerate(self.trainloader): # 遍历训练集
            ...
            if pure_ratio_discard_1 >= 0 and pure_ratio_discard_2 >= 0: # 如果纯比率丢弃1和纯比率丢弃2都大于等于0
        ...
        # 计算标签准确率,纯比率,纯比率丢弃,噪声到真实的标签,真实到噪声的标签
    ...
    def val(self, epoch=0): # 验证
        ...
        losses_B = AverageMeter()
        top1_B = AverageMeter()
        top1_mix = AverageMeter() # 混合模型的准确率,混合模型是指A模型和B模型的平均值,即(outputA + outputB) / 2,然后再进行softmax,得到混合模型的预测结果
        ...
        with torch.no_grad():
            ...
            for i, (img, label, index) in enumerate(self.valloader):
                ...
                output_mix = (outputA + outputB) / 2 # 混合模型是指A模型和B模型的平均值,即(outputA + outputB) / 2

                outputA = F.softmax(outputA, dim=1)
                outputB = F.softmax(outputB, dim=1)
                output_mix = F.softmax(output_mix, dim=1) # 对混合模型的输出进行softmax

                # Update recorder 更新记录器
                prec1_A = accuracy(outputA.data, label, topk=(1,))
                prec1_B = accuracy(outputB.data, label, topk=(1,))
                prec1_mix = accuracy(output_mix.data, label, topk=(1,)) # 混合模型的准确率，即(outputA + outputB) / 2的准确率
    ...
    def test(self, epoch=0): # 测试
        ...
        top1_mix = AverageMeter() # 混合模型的准确率
...
def save_checkpoint(state, is_best, filename='', modelbest=''): # 保存检查点
