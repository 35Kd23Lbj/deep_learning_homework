...
#   CurriculumNet: Weakly Supervised Learning from Large-Scale Web Images (CVPR 2017)
...
# 课程学习是一种弱监督学习方法，它通过逐步增加训练样本的难度来训练模型，从而提高模型的泛化能力。
# 课程学习的关键是如何确定训练样本的难度，CurriculumNet提出了一种基于密度的课程学习方法。
# 该方法首先通过计算训练样本之间的相似度来确定样本的密度，然后通过K-means算法将样本划分为多个子集，
# 每个子集代表一个难度级别。最后，通过逐步增加子集的难度来训练模型，从而提高模型的泛化能力。
...
def cluster_curriculum_subsets(X, y, n_subsets=3, method='default', density_t=0.6, verbose=False,
    ...
    # 参数：X，y，子集的数量，计算密度的方法，密度阈值，详细模式，维度缩减，最大批量大小，随机种子，计算辅助信息
...
# CurriculumClustering，课程聚类，继承BaseEstimator和ClusterMixin，实现fit和fit_predict方法
class CurriculumClustering(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, n_subsets=3, method='default', density_t=0.6, verbose=False,
        ...
        self.n_subsets = n_subsets # number of subsets，子集的数量
        self.method = method # method to calculate density，计算密度的方法
        self.density_t = density_t # density threshold，密度阈值
        self.verbose = verbose # verbose mode，详细模式
        self.output_labels = None # output labels，输出标签
        self.random_state = random_state # random state，随机种子
        self.dim_reduce = dim_reduce # dimension reduction，维度缩减
        self.batch_max = batch_max # maximum batch size，最大批量大小
        self.calc_auxiliary = calc_auxiliary # calculate auxiliary info，计算辅助信息

    def fit(self, X, y): # fit方法，用于训练模型
        X = check_array(X, accept_sparse='csr')
        check_consistent_length(X, y)
        self.output_labels, _ = cluster_curriculum_subsets(X, y, **self.get_params())
        return self

    def fit_predict(self, X, y=None): # fit_predict方法，用于训练模型并预测
        self.fit(X, y)
