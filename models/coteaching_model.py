...
class MLPNet(nn.Module): # 定义一个多层感知机网络
    def __init__(self):
...
class CNN_small(nn.Module): # 定义一个小型的卷积神经网络
...
def call_bn(bn, x): # 定义一个调用BatchNorm2d的函数
    return bn(x)

class CNN(nn.Module): # 定义一个卷积神经网络
...
class NewsNet(nn.Module): # 定义一个新闻分类网络
